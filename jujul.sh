...
homeFunction () {
	cd /home/apprenant/Documents/DFS17
		...
		echo "Pour changer les droits du fichier taper 8 "
...
rightMod () {
	echo "Dans quel dossier est le fichier dont vous voulez changer les droits: "
		ls -d */
		read dirName
		if [ -d $dirName ] 
			then
				...
				echo "Voici la liste des fichiers: "
				ls
				read -p "Quel est le fichier dont vous voulez changer les droits ? " fileToChange
				read -p "Quel droits voulez-vous mettre au fichier $fileToChange ? " fileRight
				if [ -f $fileToChange ] 
					then
						chmod $fileRight $fileToChange
						echo "Le fichier $fileToChange a changer de droits"
						homeFunction
				else
